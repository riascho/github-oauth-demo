...
import express, { Request, Response, NextFunction } from "express"; // uses '/views' as default to look for HTML templates
...
import passport, { Profile, DoneCallback } from "passport"; // Authentication middleware for Node.js
...
passport.use(
  ...
  // Github will respond with auth code
);

// Serialize user data into session - determines what data is stored in session
passport.serializeUser((user, done: DoneCallback) => {
  done(null, user as Profile); // Store entire user profile in session (in production, store just user ID)
});

// Deserialize user data from session - retrieves user data from session
passport.deserializeUser((user, done: DoneCallback) => {
  done(null, user as Profile); // Return the user object (in production, fetch full user data from database)
});
...
// Passport makes the server-to-server HTTP request internally to exchange the auth code for an access token.
// This happens on the backend, not in the browser, so it won't appear in the browser's developer tools.
...
function verifyCallback(
  ...
  // In production, you would save these to database and return user ID
